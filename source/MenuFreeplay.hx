...
/*		var bgsecret:FlxSprite = new FlxSprite(-89).loadGraphic(Paths.image('fBG_Main'));
		var thingy3 = FlxG.keys.pressed.M;
		var thingy4 = FlxG.keys.pressed.ALT;
		var donecheckthing:String = 'no';
		if (donecheckthing == 'yes')
		{
			...
			{
				inputText.text = "" + inputText + FlxG.keys.justPressed;
				inputText.screenCenter();
			}
			var thingy5 = FlxG.keys.justPressed.ENTER;
			if (thingy5)
			{
				if (inputText.text == 'idono')
				{
					donecheckthing = 'no';
					trace(inputText);
				}
			...
			var thingy6 = FlxG.keys.justPressed.ESCAPE;
			if (thingy6)
			{
				remove(bgsecret);
				FlxG.sound.music.volume += 0.5 * _variables.mvolume / 100 * FlxG.elapsed;
		...
		else if (thingy3 && thingy4)
		{
			
			FlxG.sound.music.volume = 0;
			bgsecret.scrollFactor.x = 0;
			bgsecret.scrollFactor.y = 0;
			bgsecret.setGraphicSize(Std.int(bg.width * 1.1));
			bgsecret.updateHitbox();
			bgsecret.screenCenter();
			bgsecret.antialiasing = true;
			donecheckthing = 'yes';
			inputText = new FlxText(FlxG.width * 0.7, 5, 0, "", 32);
			inputText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, RIGHT);
			inputText.alignment = CENTER;
			inputText.setBorderStyle(OUTLINE, 0xFF000000, 5, 1);
			inputText.screenCenter(X);
			inputText.y = sprDifficulty.y - 38;
			inputText.text = "Press any key to start typing";
			FlxTween.tween(FlxG.camera, {y: FlxG.height}, 1.6, {ease: FlxEase.expoIn, startDelay: 0.4});
			new FlxTimer().start(4, function(tmr:FlxTimer)
			{
				inputText.screenCenter(Y);
				bgsecret.screenCenter(Y);
				add(inputText);
				add(bgsecret);
				FlxG.height = 0;
			});
