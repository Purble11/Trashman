...
class HealthIcon extends FlxSprite
{
	...
	public function new(char:String = 'bf', isPlayer:Bool = false)
	{
		...
		else // fallback support
		{
			...
			animation.add('dirty-bf', [0, 1, 2], 0, false, isPlayer);
			...
			animation.add('trashman', [43, 44, 45], 0, false, isPlayer);
			animation.add('lilteddy', [46, 47, 48], 0, false, isPlayer);
		...
		switch (char)
		{
			case 'bf-pixel' | 'senpai' | 'senpai-angry' | 'spirit' | 'gf-pixel':
				antialiasing = false;
			//case 'trashman': //I AM TO LAZY TO PUT FIGURE OUT THE RIGHT SCALING
			//	scale.set(0.9, 0.9);
			default:
